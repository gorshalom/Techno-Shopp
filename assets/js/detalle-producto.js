...
/* producto */
let producto = window.location.href.split("=").pop();


/* galeria de imagenes */
let galerriaImg = document.getElementById("galeria-imagenes");
let carruselInner = document.getElementById("carousel-inner");
for (let i = 0; i < data.datos.imagenes.length; i++) {
    let tempImgGal = `<img src="${data.datos.imagenes[i]}" alt="" class="col-md-3 col-lg h-25  mb-3 img-gal ">`;
    ...
    tempImgCarr += `">
            <img src="${data.datos.imagenes[i]}" alt="" class="col w-auto carru">
        </div>`;
...
breadcrumbCategoria.innerHTML = data.datos.categoria;
breadcrumbCategoria.href = "/templates/mujer.html?categoria=" + data.datos.categoria;

let breadcrumbProducto = document.querySelector("#breadcrumbProducto");
breadcrumbProducto.innerHTML = data.datos.nombre;
...
titulos.forEach(function (titulo) {
    titulo.innerHTML = data.datos.nombre;
});

let precio = document.querySelector("#precio");
precio.innerHTML = `Precio: $${data.datos.precio}`;
...
resumenCalif.innerHTML += `<label for="" class="calif">${data.datos.calificacion}</label> `;

for (let index = 1; index <= 5; index++) {
    if (data.datos.calificacion >= index) {
        resumenCalif.innerHTML += estrellaLLena;
        calificacion.innerHTML += estrellaLLena;
    } else if (data.datos.calificacion >= index - .5) {
...
calificacion.innerHTML += ` (${data.datos.opiniones.length} opiniones)`;

let detalleProducto = document.querySelector("#detalleProducto");
for (let index = 0; index < data.datos.detalle.length; index++) {
    detalleProducto.innerHTML += `<li>*${data.datos.detalle[index]}</li>`;
}

let calificiones = document.querySelector("#calificaciones");
calificiones.innerHTML += `Basado en ${data.datos.opiniones.length} comentarios`;
...
for (let i = 1; i <= 5; i++) {
    ...
    let tempProgress=`<div class="progress-bar bgn-black" role="progressbar" style="width: ${data.datos.detalleCalif.porcentaje[i - 1]}%" aria-valuenow="${data.datos.detalleCalif.porcentaje[i - 1]}"
    aria-valuemin="0" aria-valuemax="100" id="progresEstrella5"></div>`;
    
    console.log(progresEstrella);
    progresEstrella.innerHTML = tempProgress;
    

    estrellaCont.innerHTML += data.datos.detalleCalif.estrellas[i - 1];
}

let opiniones = document.querySelector("#opiniones");
data.datos.opiniones.forEach(opinion => {
    let templateOpinion = `
